...
# Run all targets
.PHONY: all
all : doc/main_report.md results/out.png
...
results/out.png: Makefile
		make -Bnd | make2graph > results/output.dot
		make -Bnd | make2graph | dot -Tpng -o results/out.png
		make -Bnd | make2graph --format x > results/output.xml
